...
def _get_sigmas(config):
    ...
    """
    sigmas = np.exp(
        np.linspace(np.log(config.model.sigma_max), np.log(config.model.sigma_min), config.model.n_scales))
...
class NCSNpp(nn.Module):
    ...
    def __init__(self, config):
        ...
        if embedding_type == 'fourier':
            ...
            modules.append(layers.GaussianFourierProjection(
        ...
        if progressive == 'output_skip':
            self.pyramid_upsample = layers.Upsample(fir=fir, fir_kernel=fir_kernel, with_conv=False)
        elif progressive == 'residual':
            pyramid_upsample = functools.partial(layers.Upsample,
        ...
        if progressive_input == 'input_skip':
            self.pyramid_downsample = layers.Downsample(fir=fir, fir_kernel=fir_kernel, with_conv=False)
        elif progressive_input == 'residual':
            pyramid_downsample = functools.partial(layers.Downsample,
                                                   fir=fir, fir_kernel=fir_kernel, with_conv=True)
    ...
    def forward(self, x, time_cond):
        attn_index = -1 if x.shape[-1] <= x.shape[-2] else -2
        ...
        for i_level in range(self.num_resolutions):
            ...
            for i_block in range(self.num_res_blocks):
                ...
                m_idx += 1
                if h.shape[attn_index] in self.attn_resolutions:
                    h = modules[m_idx](h)
                    m_idx += 1
